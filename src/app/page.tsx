  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[40vh]">
        <p className="text-gray-500 font-inter">Carregando publicações...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 mt-2">
      <div className="border-b border-gray-300 pb-2 mb-6">
        <h2 className="font-inter text-xl text-[#c9302c] font-bold uppercase tracking-wide">
          Posts
        </h2>
      </div>
      ...
      <div className="grid gap-8">
        {!posts || posts.length === 0 ? (
          <div className="py-16 text-center bg-white border border-gray-200 rounded-sm">
          ...
          posts.map((post: any) => (
            <article
              key={post.id}
              className="bg-white border border-gray-200 rounded-sm p-6 shadow-sm hover:shadow-md transition-shadow duration-200 relative"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 pb-4 border-b border-gray-100 gap-4">
                <div className="flex items-center gap-3">
                  <img
                    src="/avatar.png"
                    alt="Avatar"
                    className="w-10 h-10 rounded-sm object-cover border border-gray-300 bg-gray-100"
                    onError={(e) => {
                      (e.target as HTMLImageElement).style.display = "none";
                    }}
                  />
                  <div>
                    <span className="block text-[#c9302c] font-bold text-sm uppercase tracking-wide">
                      {post.author || "Admin"}
                    </span>
                ...
                <span className="text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-1 rounded-sm border border-gray-200 uppercase tracking-wider self-start sm:self-center">
                  {post.tag}
                </span>
              </div>
              ...
              {isLoggedIn && (
                <div className="mt-4 pt-4 border-t border-gray-100 flex justify-end">
                  <button
                    ...
                    className="text-xs text-gray-400 hover:text-[#c9302c] font-medium hover:underline transition-colors"
                  >
                    Excluir publicação
                  </button>
                </div>
